...
def delete_last():
    ...
    isertar.delete(0, tk.END)
    isertar.insert(0, current_text[:-1]) 
...
def obtener_expresion(texto):
    ...
    texto = re.sub(r"(\d+)√(\d+)", r"\2**(1/\1)", texto) 
    texto = re.sub(r"√(\d+)", r"\1**(1/2)", texto) 
    texto = re.sub(r"∛(\d+)", r"\1**(1/3)", texto) 
    texto = re.sub(r"(\d+)!", r"sp.factorial(\1)", texto) 
